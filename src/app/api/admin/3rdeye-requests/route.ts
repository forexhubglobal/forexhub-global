...
export async function GET() {
  try {
    ...
    console.error(error);
    return NextResponse.json({ error: 'Gagal mendapatkan senarai 3RDEYE' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.GITHUB_TOKEN) {
      ...
      await saveToGitHub(`content/3rdeye-requests/${slug}.md`, newFrontmatter, `[skip ci] Update 3RDEYE status: ${slug} -> ${status}`);
    } else {
