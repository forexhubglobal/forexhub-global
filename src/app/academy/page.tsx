      ...
      <main className="bg-slate-50 min-h-screen py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">ForexHub Academy</h1>
          ...
          {academy.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {academy.map((article) => (
                <article key={article.slug} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-shadow group relative">
                  <Link href={`/blog/${article.slug}`}>
                    <div className={`h-48 w-full relative overflow-hidden ${!article.image?.includes('/') ? (article.image || 'bg-slate-100') : 'bg-slate-100'}`}>
