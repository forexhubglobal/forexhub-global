        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {displayBrokers.map((broker: any, index: number) => (
            ...
            >
              ...
              {/* Logo Area */}
              <div className="w-full h-20 relative mb-6 p-2 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:border-neon-blue/50 transition-colors">
                {broker.logo ? (
                  <div className="relative w-full h-full">
                    <Image src={broker.logo} alt={broker.title} fill className="object-contain drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]" />
                  </div>
                ) : (
                  <div className="text-xl font-black text-slate-500">{broker.title.substring(0, 2).toUpperCase()}</div>
                )}
              ...
              <div className="flex items-center gap-1 mb-6">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-5 h-5 ${i < Math.floor(parseFloat(broker.rating || '5')) ? 'text-gold-400 drop-shadow-[0_0_5px_rgba(251,191,36,0.8)]' : 'text-slate-700'}`} fill="currentColor" viewBox="0 0 20 20">
              ...
              <Link href={broker.slug ? `/broker/${broker.slug}` : '#'} className="block w-full py-3 px-4 bg-white/5 hover:bg-neon-blue/20 text-neon-blue text-center font-bold rounded-xl transition-all border border-white/10 group-hover:border-neon-blue/50 group-hover:shadow-[0_0_15px_rgba(0,243,255,0.4)]">
                Baca Review
              </Link>
            </div>
          ))}
